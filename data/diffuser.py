...
class Diffuser:
    ...
    def ar_reverse_diffusion(
       ...
       """ 
        ...
        #a_t = torch.tensor(a_t, device=x_t.device, dtype=x_t.dtype)
        #b_t = torch.tensor(b_t, device=x_t.device, dtype=x_t.dtype)
        #a_t = torch.tensor(a_t, device=x_t.device, dtype=x_t.dtype)
        cum_a_t = self.cum_a_schedule[t]
        ...
        #x_t_1 = 1 / np.sqrt(a_t) * (x_t - pred_noise.cpu().numpy()) + z * np.sqrt(b_t) * noise_scale
        if mask is not None:
