# -*- coding: utf-8 -*-
"""Pytorch script for training DynaRNA diffusion.
...
def get_pdb_coordinates(pdb_file, num_res_sample):
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure("RNA", pdb_file)
    
    # Get backbone mask and coordinates
    coords = []
    for model in structure:
        for chain in model:
            for i, residue in enumerate(chain):
                # Only select the first num_res_sample residues
                if i >= num_res_sample:
                    break
                if 'C4\'' in residue:
                    coords.append(residue['C4\''].get_coord())
    
    # Convert to numpy array
    coords = np.array(coords)
    bb_mask = np.ones((1, num_res_sample))  # Single sample, length is num_res_sample
    return bb_mask, coords

def gather_args():
    """Parses command line."""
    parser = argparse.ArgumentParser(prog="RNA diffusion experiment")
    parser.add_argument(
        "--disable_wandb",
        action='store_true',
...
class Experiment:
    ...
    def sample_reverse_diffusion(self, bb_mask, initial_coords=None, log_freq=100, process=True, noise_scale=1.0,rever_step=None):
        rever_step = rever_step if rever_step is not None else self._exp_cfg.T
        batch_size, seq_len = bb_mask.shape
        if initial_coords is not None:
            x_T = torch.Tensor(initial_coords).type(torch.float32)
        else:
            x_T = torch.Tensor(np.random.normal(size=[batch_size, seq_len, 3])).type(torch.float32)
        #x_T = torch.Tensor(np.random.normal(size=[batch_size, seq_len, 3])).type(torch.float32) 
        x_T *= bb_mask[..., None]
        ...
        for t in reversed(range(rever_step)): #self._exp_cfg.T)):
        ...
        return sampled_diffusion
    def sample_partial_reverse_diffusion(self, bb_mask, initial_coords, region_masks):
        """
        Apply forward and reverse diffusion to different regions with specified steps.
        """
        sampled_diffusion = []
        
        # Iterate over each region and apply diffusion
        for region, params in region_masks.items():
            start = params['start']
            end = params['end']
            t_step = params['t_step']
            rever_step = params['rever_step']
            
            # Extract the region coordinates
            coords_subset = initial_coords[:, start:end] if end is not None else initial_coords[:, start:]
            
            # Apply forward diffusion to the region
            coords_diffused, noise_t = self.diffuser.closed_form_forward_diffuse(coords_subset, t_step)
            
            # Create a mask for the current region
            inpaint_mask = np.zeros(bb_mask.shape)
            if end is None:
                inpaint_mask[:, start:] = 1
            else:
                inpaint_mask[:, start:end] = 1
            
            # Create full_region_coords by inserting coords_diffused into initial_coords
            full_region_coords = initial_coords.copy()
            if end is not None:
                full_region_coords[:, start:end] = coords_diffused
            else:
                full_region_coords[:, start:] = coords_diffused
            
            # Prepare initial features for reverse diffusion
            batch_size = bb_mask.shape[0]
            seq_len = bb_mask.shape[1]
            x_T = torch.tensor(full_region_coords, device=self.device).float()
            sample_feats = {
                'bb_mask': torch.tensor(bb_mask, device=self.device),
                'bb_corrupted_mask': torch.tensor(inpaint_mask, device=self.device),
                'residue_index': torch.tile(torch.arange(seq_len, device=self.device), (batch_size, 1)),
            }
            
            # Perform reverse diffusion on the region
            for t in reversed(range(rever_step)):
                # Update current features
                sample_feats['bb_corrupted'] = x_T
                sample_feats['t'] = torch.tensor([t] * batch_size, device=self.device)
                
                # Predict noise using the model
                output = self.model(sample_feats)
                e_t = output  # Directly use the model's output (assumes it's a tensor)
                
                # Update coordinates using reverse diffusion
                x_T = self.diffuser.ar_reverse_diffusion(
                    x_T, e_t, t, mask=torch.tensor(inpaint_mask[..., None], device=self.device),
                    noise_scale=1.0
                ).type(torch.float32)
            
            # Process the de-noised region
            processed_region = self.process_bb_pos(
                x_T.cpu().detach().numpy(),
                bb_mask[0]
            )
            sampled_diffusion.append(processed_region)
        
        # Combine de-noised regions into a single array
        composite_diffusion = np.zeros_like(initial_coords)
        for region, diffusion in zip(region_masks.values(), sampled_diffusion):
            start = region['start']
            end = region['end']
            current_region = diffusion
            
            # Update the composite diffusion array
            if end is None:
                composite_diffusion[:, start:] = current_region
            else:
                composite_diffusion[:, start:end] = current_region
        
        return composite_diffusion
    def inpaint_reverse_diffusion(
            self, initial_pos, bb_mask, inpaint_mask,
            log_freq=100, process=True, noise_scale=1.0, rever_step=None):
